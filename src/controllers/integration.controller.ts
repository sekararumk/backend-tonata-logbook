...
const verifyAuthToken = (authHeader: string | undefined) => {
  ...
  try {
    ...
    if (token) {
      try {
        ...
        console.log('✅ Token verified for user:', (currentUser as any).username);
      } catch (err) {
    ...
    const transformedData = result.rows.map(row => {
      ...
      const isOwner = currentUser && (
        (currentUser as any).username === row.owner_username ||
        (currentUser as any).id_pengguna === row.id_pengguna
      );
      ...
      return {
        ...
        current_user: currentUser ? (currentUser as any).username : null,
    ...
    if (currentUser) {
      console.log('👤 Current user:', (currentUser as any).username);
...
export const addLogbook = async (req: Request, res: Response) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT id_pengguna, nama_pengguna FROM pengguna WHERE username = $1',
    ...
    res.status(201).json({
      ...
      data: {
        ...
        nama: userResult.rows[0].nama_pengguna, // Ambil nama dari user yang login
        judul: newLogbook.judul_logbook,
        keterangan: newLogbook.keterangan,
        link: newLogbook.link,
        canEdit: true, // User yang buat pasti bisa edit
        canDelete: true,
        canView: true,
...
export const handleTableAction = async (req: Request, res: Response) => {
  try {
    ...
    switch (action) {
      ...
      case 'edit':
        if (!isOwner) return res.status(403).json({ success: false, error: 'Permission denied' });
        if (!data || !data.judul || !data.keterangan) return res.status(400).json({ success: false, error: 'Missing required fields for edit' });
        
        // PERBAIKAN UTAMA: Menggunakan nama kolom yang benar
        const updateResult = await pool.query(`
          UPDATE logbook
          SET tanggal = $1, judul_logbook = $2, keterangan = $3, link = $4, updated_at = NOW()
          WHERE id_logbook = $5
          RETURNING *
        `, [data.date || logbook.tanggal, data.judul, data.keterangan, data.link, id]);

        res.json({ success: true, message: 'Logbook updated', data: updateResult.rows[0] });
        break;
...
export const handleFormLogin = async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign(
      { id_pengguna: user.id_pengguna, username: user.username, nama_pengguna: user.nama_pengguna },
      JWT_SECRET,
      { expiresIn: '24h' }
    );
